...
class Node:
    """Nodo dell'albero binario di Huffman."""
    ...
    # Necessario per heapq (ordine per frequenza crescente)
...
class Code:
    """
    Genera e gestisce codici binari prefisso ottimali tramite l'algoritmo di Huffman.
    ...
    sola lettura dopo l'inizializzazione):
        ...
        code            Dict[str, str]
            Codebook Huffman: {simbolo → codeword binaria}.
        ...
        coding_type     str
            Tipo di algoritmo di codifica. Valore fisso: ``'Huffman coding'``.
    ...
    def __init__(
        ...
        self.coding_type : str       = "Huffman coding"
        # Percorso del file JSON associato al codebook (None finché non si chiama write_code/read_code)
        ...
        self.code           : Dict[str, str]   = self._huffman_coding()
    ...
    def _huffman_coding(self) -> Dict[str, str]:
        """
        Costruisce il codebook Huffman ottimale.

        Algoritmo:
            1. Crea un nodo foglia per ogni simbolo.
            2. Inserisce tutti i nodi in un min-heap (per frequenza).
            3. Estrae i due nodi minimi, li unisce in un nodo interno
               con frequenza pari alla somma, reinserisce nell'heap.
            4. Ripete finché rimane un solo nodo (la radice).
            5. Visita l'albero: sinistra → '0', destra → '1'.
        ...
        if len(self.frequencies) == 1:
            symbol = next(iter(self.frequencies))
            return {symbol: "0"}

        heap: list[Node] = []
        for symbol, freq in self.frequencies.items():
            heapq.heappush(heap, Node(symbol=symbol, freq=freq))
        ...
        code: Dict[str, str] = {}
        self._generate_codes(heap[0], "", code)
        return code

    def _generate_codes(self, node: Node, prefix: str, code: Dict[str, str]) -> None:
        """
        Visita ricorsiva dell'albero di Huffman per assegnare le codeword.

        Args:
            node:   Nodo corrente.
            prefix: Stringa di bit accumulata dalla radice al nodo corrente.
            code:   Dizionario popolato in-place con {simbolo: codeword}.
        """
        if node is None:
            return
        if node.is_leaf():
            code[node.symbol] = prefix if prefix else "0"
            return
        self._generate_codes(node.left,  prefix + "0", code)
        self._generate_codes(node.right, prefix + "1", code)
    ...
    def encode(self, text: str) -> str:
        """
        Codifica una stringa tramite il codebook Huffman.
    ...
    def decode(self, binary_string: str) -> str:
        """
        Decodifica una stringa di bit tramite il codebook Huffman (approccio greedy).
    ...
    def introduce_yourself(self) -> None:
        ...
        Esempio di output::
            ...
            Tipo di codifica : Huffman coding
    ...
    def code_table(self) -> str:
        ...
        lines = [
            "",
            SEP,
            "TABELLA DEL CODICE BINARIO (Huffman)",
            SEP,
    ...
    def load_code(cls, code_file: str) -> "Code":
        ...
        instance.coding_type    = "Huffman coding"
